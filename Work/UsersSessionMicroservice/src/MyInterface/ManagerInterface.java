...
public interface ManagerInterface {
    ...
    public default String Execute(String query, Connection connection_with_db, Gson gson) throws SQLException, MyParseException, MyNotAuthorizedException {
        Statement StatementWithDB = null;
        int res = -1;
        
        StatementWithDB = connection_with_db.createStatement();
        StatementWithDB.execute(query);
        StatementWithDB.getResultSet().next();
        res = StatementWithDB.getResultSet().getInt(1);
        StatementWithDB.close();

        return gson.toJson(new SQLResult(String.valueOf(res)));
    }
    ...
    public default <T extends ModelInterface> String Update(T item, String table, Connection connection_with_db, Gson gson) throws SQLException, MyParseException, MyNotAuthorizedException {
        ...
        statement_with_db.close();
        return gson.toJson(new SQLResult("Ok", item.getId(), item.getUser_id()));
    }
}
